...
public class StudentManager extends Application {
	...
	public void start(Stage primaryStage, Scene previousScene) {
		...
		dateOfBirthColumn.setCellValueFactory(
				data -> new javafx.beans.property.SimpleObjectProperty<>(data.getValue().getDateOfBirth()));

		TableColumn<Student, String> placeOfBirthColumn = new TableColumn<>("Place of Birth");
		placeOfBirthColumn.setMinWidth(150);
		placeOfBirthColumn.setCellValueFactory(
				data -> new javafx.beans.property.SimpleStringProperty(data.getValue().getPlaceOfBirth()));
	...
	// Add student method with duplicate check
	private void addStudent() {
		try {
			...
			// Add the student if no duplicates were found
	...
	// Edit student method with duplicate check
	private void editStudent() {
		...
		try {
			long newId = Long.parseLong(idInput.getText());
			String newName = nameInput.getText();
			LocalDate newDob = dateOfBirthInput.getValue();
			String newPlace = placeOfBirthInput.getText();

			if (newName.isEmpty() || newDob == null || newPlace.isEmpty()) {
				showAlert("Please fill in all fields.");
				return;
			}

			// Check if the new ID already exists in other students
			for (Student student : students) {
				if (student != selectedStudent && student.getStudentId() == newId) {
					showAlert("A student with this ID already exists.");
					return;
				}
			}
			...
			selectedStudent.setDateOfBirth(newDob);
			selectedStudent.setPlaceOfBirth(newPlace);
			saveStudentsToFile(); // Save changes
			table.refresh();
			clearFields();
		} catch (NumberFormatException e) {
	...
	private void saveStudentsToFile() {
		try (BufferedWriter writer = new BufferedWriter(new FileWriter(FILE_PATH))) {
			for (Student student : students) {
				writer.write(student.getStudentId() + "," + student.getFullName() + "," + student.getDateOfBirth() + ","
